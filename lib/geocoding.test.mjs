...
test("invalid Photon features are ignored", () => {
  assert.equal(
    mapPhotonFeature({ properties: { name: "Missing point" } }),
    null,
  );
});

test("search restricts onboarding coverage to Ghana and South Africa", async () => {
  ...
  try {
    ...
    assert.deepEqual(new URL(requestedUrl).searchParams.getAll("countrycode"), [
      "GH",
      "ZA",
    ]);
  } finally {
